...
(function () {
  ...
  /* 2) Inline hero/page form — plain GHL embed (no skeleton, no lazy load).
        The iframe carries its real src DIRECTLY in the page HTML, so the form is always
        present, renders in place, and (via its inline min-height) keeps its section height
        reserved so it never collapses/disappears — mobile included. The only JS here is
        loading GHL's form_embed.js ONCE per page so the iframe auto-resizes to its content.
        PERF: form_embed runs in THIS page and its initial resize is what previously lagged
        the first taps on the homepage hero. The form is already visible without it, so we
        load it a beat after first paint — on the first scroll OR a short timeout, whichever
        comes first — never on a tap, and with no placeholder/"loading" text of any kind. */
  (function armFormEmbed() {
    if (!document.querySelector('iframe[id^="inline-"]')) return;   // page has no inline form
    var done = false;
    function go() {
      if (done) return;
      done = true;
    ...
    setTimeout(go, 1200);   // load even without a scroll, just past the initial-paint/tap window
  })();
